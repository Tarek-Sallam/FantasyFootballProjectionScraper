...
def pull_nfl_data(path) -> None:
    ...
    # export a dataframe with the data
    data = pd.DataFrame({"name": names, 'position': pos, 'proj': pts})
    data.to_csv(os.path.join(path, "nfl_raw.csv"), index=False)
